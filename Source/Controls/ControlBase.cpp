...
ControlBase::ControlBase(IHAL* HAL, uint16_t UpdateRate)
	: m_HAL(HAL),
	m_Enabled(true),
...
void ControlBase::SetPinMode(uint8_t Pin, IHAL::PinModes Mode)
{
	m_HAL->SetPinMode(Pin, Mode);
}

float ControlBase::AnalogRead(uint8_t Pin) const
{
	return m_HAL->AnalogRead(Pin);
}

bool ControlBase::DigitalRead(uint8_t Pin) const
{
	return m_HAL->DigitalRead(Pin);
}

void ControlBase::DigitalWrite(uint8_t Pin, bool Value)
{
	m_HAL->DigitalWrite(Pin, Value);
}

void ControlBase::PWMWrite(uint8_t Pin, float Value)
{
