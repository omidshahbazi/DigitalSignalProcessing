...
public:
	...
	static void FastFourierTransform(Complex *Buffer)
	{
...
#else
		for (uint16_t i = 1, j = 0; i < SampleCount; i++)
		{
			uint16_t bit = SampleCount >> 1;
			for (; j & bit; bit >>= 1)
				j ^= bit;
			j ^= bit;
		...
		for (uint16_t len = 2; len <= SampleCount; len <<= 1)
		{
			float ang = -Math::TWO_PI_VALUE / (float)len;
			Complex wlen(Math::Cos(ang), Math::Sin(ang));
			for (uint16_t i = 0; i < SampleCount; i += len)
			{
				Complex w(1, 0);
	...
	template <typename T, uint32_t SampleRate, uint16_t SampleCount>
	static Frequency CalculateFrequencyRaw(const T *const RignBuffer, uint16_t CurrentIndex, float Threshold = 0.05)
	{
